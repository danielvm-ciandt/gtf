...
export function createWorktree(basePath, name, opts = {}) {
    ...
    if (branchAlreadyExists) {
        ...
        if (branchInUse) {
            throw new GSDError(GSD_LOCK_HELD, `Branch "${branch}" is already in use by another worktree. ` +
...
export function listWorktrees(basePath) {
    ...
    for (const entry of entries) {
        ...
        if (!matchedRoot && !matchesBranchLeaf)
            continue;
        const matchedEntryPath = normalizedEntryVariants.find(entryVariant => matchedRoot ? entryVariant.startsWith(`${matchedRoot.normalized}/`) : false);
