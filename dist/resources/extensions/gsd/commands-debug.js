...
export function parseDebugCommand(args) {
    ...
    if (head === "status") {
        if (parts.length === 1)
            return { type: "error", message: "Missing slug. Usage: /gsd debug status <slug>" };
    ...
    if (head === "continue") {
        if (parts.length === 1)
            return { type: "error", message: "Missing slug. Usage: /gsd debug continue <slug>" };
        if (parts.length === 2 && isValidSlugCandidate(parts[1]))
...
export async function handleDebug(args, ctx, pi) {
    ...
    if (parsed.type === "issue-start") {
        ...
        try {
            ...
            ctx.ui.notify(`Unable to create debug session: ${message}\nTry /gsd debug --diagnose for artifact health details.`, "error");
        }
        return;
    }
    if (parsed.type === "list") {
    ...
    if (parsed.type === "status") {
        try {
            ...
            if (!loaded) {
                ctx.ui.notify(`Unknown debug session slug '${parsed.slug}'. Run /gsd debug list to see available sessions.`, "warning");
                return;
            }
    ...
    if (parsed.type === "continue") {
        try {
            ...
            if (loaded.session.status === "resolved") {
                ctx.ui.notify(`Session '${parsed.slug}' is resolved. Open a new session with /gsd debug <issue-text> for follow-up work.`, "warning");
                return;
            }
            ...
            if (canDispatch) {
                try {
                    ...
                    pi.sendMessage({ customType: "gsd-debug-continue", content: prompt, display: false }, { triggerTurn: true });
                }
                catch (err) {
                    const msg = err instanceof Error ? err.message : String(err);
                    ctx.ui.notify(`Continue dispatch failed: ${msg}\nSession '${resumed.session.slug}' is persisted; retry with /gsd debug continue ${resumed.session.slug}`, "warning");
                }
            }
        }
        catch (error) {
        ...
        try {
            ...
            if (pi && typeof pi.sendMessage === "function") {
                try {
                    ...
                    pi.sendMessage({ customType: "gsd-debug-diagnose", content: prompt, display: false }, { triggerTurn: true });
                }
                catch (err) {
                    const msg = err instanceof Error ? err.message : String(err);
                    ctx.ui.notify(`Diagnose dispatch failed: ${msg}\nSession '${s.slug}' is persisted; continue manually with /gsd debug continue ${s.slug}`, "warning");
                }
            }
        }
        catch (error) {
