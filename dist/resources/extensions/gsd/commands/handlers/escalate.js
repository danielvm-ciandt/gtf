...
import { readEscalationArtifact, formatEscalationForDisplay, resolveEscalation, listActionableEscalations, listAllEscalations, } from "../../escalation.js";
...
export async function handleEscalateCommand(args, ctx, pi) {
    void pi;
    ...
    if (prefs?.phases?.mid_execution_escalation !== true) {
        ctx.ui.notify("Escalation is off. Enable with `phases: { mid_execution_escalation: true }` in your PREFERENCES.md.", "warning");
        return;
    }
    ...
    if (trimmed === "list" || trimmed === "list --all" || trimmed === "--all") {
        ...
        ctx.ui.notify(`${includeAll ? "All escalations" : "Actionable escalations"} for ${milestoneId}:\n${body}`, "info");
        return;
    }
    ...
    const locateRow = (ref) => {
        ...
        const rows = listAllEscalations(milestoneId).filter((t) => t.id === taskId && (sliceId === undefined || t.slice_id === sliceId));
        if (rows.length === 0)
            return "not-found";
        if (rows.length > 1)
            return "ambiguous";
    ...
    if (trimmed.startsWith("resolve ")) {
        ...
        if (result.status === "resolved") {
            try {
                ...
                ctx.ui.notify(`${result.message}\nDecision recorded as ${decisionId}. Run /gsd auto to continue.`, "success");
            }
            catch (decErr) {
                ctx.ui.notify(`${result.message}\nWARN: decision persistence failed: ${decErr.message}`, "warning");
            }
            return;
        }
