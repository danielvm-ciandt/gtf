 ...
 */
export function parseWorkflowOverridesOnly(args) {
    const parts = splitWorkflowRunArgs(args);
    const overrides = {};
...
function dispatchPluginByMode(plugin, args, ctx, pi) {
    switch (plugin.meta.mode) {
        ...
        case "auto-milestone": {
            ctx.ui.notify(`'${plugin.name}' runs via the full milestone pipeline.\n` +
                `Use /gsd auto or /gsd start ${plugin.name}.`, "info");
            return;
        }
    }
}
async function handleCustomWorkflow(sub, ctx, pi) {
    ...
    if (head === "install") {
        if (!rest) {
            ctx.ui.notify("Usage: /gsd workflow install <source> [--project] [--name <n>]\n\n" +
                "Sources:\n" +
                "  https://…/path/workflow.yaml\n" +
                "  gist:<id>\n" +
                "  gh:owner/repo/path[@ref]", "warning");
            return true;
        }
        ...
        try {
            ...
            if (!name)
                throw new Error("Could not infer plugin name. Use --name <n>.");
            const target = scope === "global"
                ? { scope: "global", dir: globalInstallDir() }
                : { scope: "project", dir: projectInstallDir(base) };
            ...
            ctx.ui.notify(`✓ Installed plugin "${result.name}" (${result.ext.slice(1)}) to ${result.path}`, "info");
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
    ...
    if (head === "uninstall") {
        ...
        if (!result.removed) {
            ctx.ui.notify(`No installed plugin named "${rest}" found in ${globalInstallDir()} or ${projectInstallDir(base)}.`, "warning");
            return true;
        }
    ...
    if (head === "validate") {
        ...
        if (plugin && plugin.format === "yaml") {
            try {
                ...
                ctx.ui.notify(`Failed to read definition: ${err instanceof Error ? err.message : String(err)}`, "error");
                return true;
            }
        }
        else {
