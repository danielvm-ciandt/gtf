...
import { resolveByName, autoDetect, listTemplates, getTemplateInfo, loadWorkflowTemplate, loadRegistry, } from "./workflow-templates.js";
...
export async function handleStart(args, ctx, pi) {
    ...
    if (isAutoActive()) {
        ctx.ui.notify("Cannot start a workflow template while auto-mode is running.\n" +
            "Run /gsd pause first, then /gsd start.", "warning");
        return;
    }
    if (isAutoPaused()) {
        ctx.ui.notify("Auto-mode is paused. Starting a workflow template will run independently.\n" +
            "The paused auto-mode session can be resumed later with /gsd auto.", "info");
    }
    // ─── Resume detection ───────────────────────────────────────────────────
    ...
    if (!match && cleanedArgs) {
        ...
        if (detected.length === 1 || (detected.length > 0 && detected[0].confidence === "high")) {
            ...
            const choices = detected.slice(0, 4).map((m) => `  /gsd start ${m.id} ${cleanedArgs}`);
            ctx.ui.notify(`Multiple templates could match. Pick one:\n\n${choices.join("\n")}\n\nOr specify explicitly: /gsd start <template> <description>`, "info");
            return;
        }
    }
    ...
    if (!match) {
        if (!trimmed) {
            ctx.ui.notify("Usage: /gsd start <template> [description]\n\n" +
                "Templates:\n" +
                ...
                "Run /gsd templates for detailed template info.", "info");
        }
        else {
            ctx.ui.notify(`No template matched "${firstWord}". Run /gsd start to see available templates.`, "warning");
        }
        return;
    }
    // ─── Resolved template ───────────────────────────────────────────────────
    ...
    if (templateId === "full-project") {
        ...
        if (!existsSync(root)) {
            ctx.ui.notify("Routing to /gsd init for full project setup...", "info");
            ...
            ctx.ui.notify("Project already initialized. Use `/gsd auto` to continue or `/gsd discuss` to start a new milestone.", "info");
        }
        return;
    }
    ...
    if (artifactDir) {
        writeWorkflowState(join(basePath, artifactDir), templateId, template.name, template.phases, description, actualBranch);
    }
    // ─── Notify and dispatch ────────────────────────────────────────────────
    const infoLines = [
...
export async function handleTemplates(args, ctx) {
    ...
    if (trimmed.startsWith("info ")) {
        ...
        if (info) {
            ...
            ctx.ui.notify(`Unknown template "${name}". Run /gsd templates to see available templates.`, "warning");
        }
        return;
    }
 ...
 */
export function dispatchMarkdownPhasePlugin(plugin, description, ctx, pi) {
    if (plugin.meta.mode !== "markdown-phase")
        return;
    if (isAutoActive()) {
        ctx.ui.notify("Cannot start a markdown-phase workflow while auto-mode is running.\n" +
            "Run /gsd pause first.", "warning");
        return;
    }
    ...
    try {
        ...
        ctx.ui.notify(`Failed to read template: ${err instanceof Error ? err.message : String(err)}`, "error");
        return;
    }
    // Create artifact directory.
    let artifactDir = "";
    ...
    if (!skipBranch) {
        try {
            ...
            ctx.ui.notify(`Could not create branch ${branchName}: ${getErrorMessage(err)}. Working on current branch.`, "warning");
        }
    }
    const actualBranch = branchCreated ? branchName : git.getCurrentBranch();
    // Write STATE.json.
    if (artifactDir && plugin.meta.phases && plugin.meta.phases.length > 0) {
        writeWorkflowState(join(basePath, artifactDir), templateId, plugin.meta.displayName, plugin.meta.phases, description, actualBranch);
    }
    const infoLines = [
