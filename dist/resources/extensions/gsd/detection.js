...
export function detectProjectSignals(basePath) {
    ...
    const springBootBuildFiles = scannedFiles.filter((file) => file.endsWith("pom.xml") || file.endsWith("build.gradle") || file.endsWith("build.gradle.kts"));
...
function detectPackageManager(basePath) {
    ...
    if (existsSync(join(basePath, "yarn.lock")))
        return "yarn";
    if (existsSync(join(basePath, "bun.lockb")) || existsSync(join(basePath, "bun.lock")))
        return "bun";
...
export function isFirstEverLaunch() {
    ...
    if (existsSync(join(gsdHome, "agent", "auth.json")))
        return false;
