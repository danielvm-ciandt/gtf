...
export async function handleTriage(ctx, pi, basePath) {
...
export async function handleKnowledge(args, ctx) {
    ...
    if (!typeArg || !["rule", "pattern", "lesson"].includes(typeArg)) {
        ctx.ui.notify("Usage: /gsd knowledge <rule|pattern|lesson> <description>\nExample: /gsd knowledge rule Use real DB for integration tests", "warning");
        return;
    }
