...
function renderGanttView(data, th, width) {
    ...
    for (const unit of recent) {
        ...
        const phaseColor = phase === "research" ? "dim" :
...
export function renderCapturesView(data, th, width) {
    ...
    for (const entry of sorted) {
        const statusColor = entry.status === "pending" ? "warning" :
            entry.status === "triaged" ? "accent" :
                "dim";
        const classColor = entry.classification === "inject" ? "warning" :
