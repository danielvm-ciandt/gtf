    ...
    for (const dir of syncedDirs) {
        try {
            if (existsSync(dir)) {
                ...
                const untrackedOutput = execFileSync("git", ["ls-files", "--others", "--exclude-standard", dir], { cwd: basePath, stdio: ["ignore", "pipe", "pipe"], encoding: "utf-8" }).trim();
                if (untrackedOutput) {
...
export function mergeMilestoneToMain(originalBasePath_, milestoneId, roadmapContent) {
    ...
    if (!mergeResult.success) {
        ...
        if (mergeResult.conflicts.includes("__dirty_working_tree__")) {
            ...
            throw new GSDError(GSD_GIT_ERROR, `Squash merge of ${milestoneBranch} rejected: working tree has dirty or untracked files ` +
    ...
    if (nothingToCommit) {
        ...
        if (codeChanges.length > 0) {
            ...
            process.chdir(previousCwd);
            throw new GSDError(GSD_GIT_ERROR, `Squash merge produced nothing to commit but milestone branch "${milestoneBranch}" ` +
