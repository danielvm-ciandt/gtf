 ...
 */
export async function handleCommand(text, sender, basePath) {
...
async function buildStatus(basePath) {
    ...
    if (autoData && (autoData.active || autoData.paused)) {
        ...
        if (pausedMeta) {
            ...
            if (pausedMeta.milestoneId)
                lines.push(`Milestone: ${pausedMeta.milestoneId}`);
            if (pausedMeta.unitType && pausedMeta.unitId) {
                lines.push(`Unit: ${pausedMeta.unitType} / ${pausedMeta.unitId}`);
            }
            if (pausedMeta.pausedAt)
                lines.push(`Paused at: ${pausedMeta.pausedAt}`);
        }
        else {
...
async function buildProgress(basePath) {
    ...
    const open = milestones.filter(m => !done.includes(m) && !active.includes(m) && m.status !== "parked");
...
async function buildPause(basePath) {
    try {
        ...
        capturesMod.markCaptureResolved(basePath, id, "stop", "Pause requested via Telegram", "User sent /pause command via Telegram remote channel");
...
async function readMilestonesFromDb() {
    try {
        const dbMod = await tryImportModule("../gsd/gsd-db.js");
