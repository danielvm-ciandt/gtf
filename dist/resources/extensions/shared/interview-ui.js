...
export async function showInterviewRound(questions, opts, ctx) {
    return ctx.ui.custom((tui, theme, _kb, done) => {
        ...
        function questionHasAnyPreview() {
            return questions[currentIdx].options.some((o) => o.preview != null && o.preview.trim().length > 0);
        }
        ...
        function renderOptionsColumn(optWidth) {
            ...
            if (st.notesVisible || focusNotes) {
                ...
                if (focusNotes) {
                    for (const line of getEditor().render(optWidth - 2))
                        col.push(truncateToWidth(` ${line}`, optWidth));
                }
                else if (st.notes) {
        ...
        function render(width) {
            ...
            if (useSideBySide) {
                ...
                while (leftLines.length < maxBody)
                    leftLines.push("");
