...
function pruneStaleSiblingFiles(srcDir, destDir) {
    if (!existsSync(destDir))
        return;
    const sourceFiles = new Set(readdirSync(srcDir, { withFileTypes: true })
...
function migrateSkillsToEcosystemDir(agentDir) {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (existsSync(target))
                continue; // ecosystem version wins
            candidates++;
            try {
                if (isSymlink) {
        ...
        try {
            writeFileSync(markerFd, `Migrated ${migrated} skill(s) to ${ecosystemDir} on ${new Date().toISOString()}\n`);
        }
        catch { /* non-fatal */ }
    }
    catch {
...
export function hasStaleCompiledExtensionSiblings(extensionsDir, sourceDir = bundledExtensionsDir) {
    if (!existsSync(extensionsDir))
        return false;
    const sourceFiles = existsSync(sourceDir)
        ? new Set(readdirSync(sourceDir, { withFileTypes: true })
            .filter((entry) => entry.isFile())
            .map((entry) => entry.name))
        : new Set();
    for (const entry of readdirSync(extensionsDir, { withFileTypes: true })) {
        ...
        if (!existsSync(join(extensionsDir, siblingName)))
            continue;
...
function getBundledExtensionKeys() {
    if (!_bundledExtensionKeys) {
        _bundledExtensionKeys = new Set(discoverExtensionEntryPaths(bundledExtensionsDir).map((entryPath) => getExtensionKey(entryPath, bundledExtensionsDir)));
    }
