...
export function writeInstanceRegistry(registry, registryPath = WEB_INSTANCES_PATH) {
    writeFileSync(registryPath, JSON.stringify(registry, null, 2), 'utf8');
}
export function registerInstance(cwd, entry, registryPath = WEB_INSTANCES_PATH) {
...
export function unregisterInstance(cwd, registryPath = WEB_INSTANCES_PATH) {
    const registry = readInstanceRegistry(registryPath);
...
export function deletePidFile(filePath) {
    try {
        unlinkSync(filePath);
    }
    catch {
...
export function stopWebMode(deps = {}, options = {}) {
...
export async function reserveWebPort(host = DEFAULT_HOST) {
...
async function waitForBootReady(url, timeoutMs = 180_000, stderr, authToken) {
...
export async function launchWebMode(options, deps = {}) {
    const stderr = deps.stderr ?? process.stderr;
    ...
    const spawnSpec = buildSpawnSpec(resolution, host, port, deps.platform ?? process.platform, deps.execPath ?? process.execPath);
    stderr.write(`[gsd] Launching web host on port ${port}…\n`);
    const spawnResult = await spawnDetachedProcess(deps.spawn ?? ((command, args, spawnOptions) => spawn(command, args, spawnOptions)), spawnSpec.command, spawnSpec.args, {
        cwd: spawnSpec.cwd,
        detached: true,
        stdio: 'ignore',
        windowsHide: true,
    ...
    try {
        const bootReadyFn = deps.waitForBootReady ?? ((u) => waitForBootReady(u, 180_000, stderr, authToken));
