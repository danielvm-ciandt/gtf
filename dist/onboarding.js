...
export async function runWebSearchStep(p, pc, authStorage, isAnthropicAuth) {
    ...
    options.push({ value: 'brave', label: 'Brave Search', hint: 'requires API key — brave.com/search/api' }, { value: 'tavily', label: 'Tavily', hint: 'requires API key — tavily.com' }, { value: 'skip', label: 'Skip for now', hint: 'use /search-provider inside GSD later' });
...
export async function runRemoteQuestionsStep(p, pc, authStorage) {
    ...
    if (choice === 'slack') {
        ...
        const channelId = await p.text({
            ...
            validate: (val) => {
                if (!val || !/^[A-Z0-9]{9,12}$/.test(val.trim()))
                    return 'Expected 9-12 uppercase alphanumeric characters';
            },
        });
...
async function runDiscordChannelStep(p, pc, token) {
    ...
    if (channelChoice === MANUAL_VALUE) {
        const manualId = await p.text({
            ...
            validate: (val) => {
                if (!val || !/^\d{17,20}$/.test(val.trim()))
                    return 'Expected 17-20 digit numeric ID';
            },
        });
        if (p.isCancel(manualId) || !manualId)
            return null;
