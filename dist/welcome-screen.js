...
function readGsdState() {
    try {
        ...
        if (milestone)
            state.milestone = milestone[1].trim();
        const slice = raw.match(/^\*\*Active Slice:\*\*\s*(.+)$/m);
        if (slice)
            state.slice = slice[1].trim();
        const phase = raw.match(/^\*\*Phase:\*\*\s*(.+)$/m);
        if (phase)
            state.phase = phase[1].trim();
        ...
        if (nextMatch)
            state.nextAction = nextMatch[1].trim();
        return state;
    }
    catch {
        return undefined;
...
export function printWelcomeScreen(opts) {
    ...
    if (state?.milestone) {
        ...
        if (activeText.length > maxActiveLen)
            activeText = activeText.slice(0, maxActiveLen - 1) + '…';
